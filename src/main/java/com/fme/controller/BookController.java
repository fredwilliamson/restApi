...
public class BookController {
    ...
    protected static final BookDTO MONTE;
    protected static final BookDTO ADIEU_ARMES;
    protected static final List<BookDTO> ALL_BOOKS;

    static {
        ADIEU_ARMES = new DetailBook(new BookDTO("L'Adieu aux armes", "22.3", new Author("Ernest", "Hemingway"), "1"),
        ...
        MONTE = new DetailBook(new BookDTO("Le compte de Monte-Cristo", "33.5", new Author("Alexandre", "Dumas"), "2"),
    ...
    public BookDTO greeting(@RequestParam(value = "name", defaultValue = "World") String name) {
        return MONTE;
    }
    ...
    public DetailBook getDetailBook(@PathVariable(value = "id") final String id) {
        ...
        final List<BookDTO> listById = ALL_BOOKS.stream().filter(book -> id.equals(book.getId()))
    ...
    public List<BookDTO> getBooksByAuthor(@PathVariable(value = "author") final String author) {
    ...
    public List<Author> getAuthorByName(@RequestParam(value = "name") final String name) {
        ...
        return ALL_BOOKS.stream().filter(book -> book.getAuthor().getLastName().startsWith(name))
                .map(BookDTO::getAuthor).distinct().collect(Collectors.toList());
    }
